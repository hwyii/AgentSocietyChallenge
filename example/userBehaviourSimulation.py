        ...
        try:
            ...
            for sub_task in plan:
                if 'user' in sub_task['description']:
                    user = str(self.interaction_tool.get_user(user_id=self.task['user_id'])) # 对于sub_task，返回user和item_id
            ...
            for review in reviews_item:
                ...
                self.memory(f'review: {review_text}') # 使用memory module存储review，以便后续通过similarity检索meomory
            reviews_user = self.interaction_tool.get_reviews(user_id=self.task['user_id']) # 通过user_id来获取这个人之前做的一些review
            review_similar = self.memory(f'{reviews_user[0]["text"]}') # 基于这个人的review从memory里取出相关的对item的review
            task_description = f'''
