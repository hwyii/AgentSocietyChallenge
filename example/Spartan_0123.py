    ...
    def workflow(self):
        try:
            ...
            if user['source'] == 'yelp':
                ...
                BusinessParking = attributes.get("BusinessParking", "Unknown")
                BusinessParking = BusinessParking.replace("u'", "'").replace('u"', '"')
                if BusinessParking != "Unknown":
                    BusinessParking = ast.literal_eval(BusinessParking)
            
                # Process parking information if available
                parking_info = ""
                if BusinessParking != "Unknown":
                    parking_options = []
                    for key, value in BusinessParking.items():
                        if value:  # Only include True values
                            parking_options.append(key.capitalize())
                    if parking_options:
                        parking_info = ", ".join(parking_options)
                    else:
                        parking_info = "No parking available"


                ambience = attributes.get("Ambience", "Unknown")
                if ambience != "Unknown":
                ...
                if ambience != "Unknown":
                    ambience_options = []
                    for key, value in ambience.items():
                        if value:  # Only include True values
                            ambience_options.append(key.capitalize())
                    if ambience_options:
                        ambience_info = ", ".join(ambience_options)
                    else:
                        ambience_info = "No ambience information available"
                ...
                if attributes:
                    ...
                    task_description += "\n3. Environment\n"
                    if ambience != "Unknown":
                        task_description += f"- Ambience Type: {ambience_info}\n"
                    ...
                    if BusinessParking != "Unknown":
                        task_description += f"- Parking: {parking_info}\n"
